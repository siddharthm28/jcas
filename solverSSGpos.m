function [model, progress] = solverSSGpos(param, options)
...
for p=1:options.num_passes
    ...
    for dummy = 1:n
        ...
        tmp = (1-gamma)*model.w + gamma*n * w_s; % stochastic subgradient update (notice the factor n here)
        tmp(tmp<0)=0;   % project onto the space of w >=0 
        model.w = tmp;
