function testing_in_parallel(obj)
if ~obj.destpathmade
...
matlabpool('open',4);
extract_features_in_parallel(obj,'test');
matlabpool('close');
...
if(~isfield(obj.unary,'precomputed') || ~obj.unary.precomputed)
    ...
    %set
    matlapbool('open',4);
    build_superpixels_histograms_in_parallel(obj,'test');
    matlabpool('close');
    ...
    %Compute unary costs
    matlabpool('open',4);
    compute_unary_costs_in_parallel(obj,'test');
    matlabpool('close');
else
