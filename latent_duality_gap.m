function [gap, w_s, ell_s] = latent_duality_gap( param, maxOracle, model, lambda )
...
% w_s = 1/(lambda*n) \psi_i(ystar_i)
    ...
    for i=1:n
        ...
        ystars{i} = maxOracle(param, model, patterns{i}, [labels{i};param.words{i}']);
    end
    
    w_s = zeros(size(w));
    ell_s = 0; % note that we will normalize at the end to save rounding...
    for i=1:n
        w_s = w_s + (phi(param, patterns{i}, [labels{i};param.words{i}'])-phi(param, patterns{i}, ystars{i}));
